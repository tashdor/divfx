...
v1.0.1 describes the DCP as a deliverable in the [production workflow](../production-workflow.md#dcp-digital-cinema-package) — a compressed, usually encrypted package muxed from the DCDM, with KDMs for encrypted playback. That is accurate as far as it goes. What it does not cover is everything an independent production actually trips over getting a film onto a festival screen: whether to encrypt at all, the accessibility tracks festivals now expect, how the package physically or electronically reaches the venue, and why none of it is trusted until it has played off a real cinema server.

## What a DCP is

Structurally the DCP is a close cousin of the [IMF](imf.md): a set of MXF track files described by a **CPL** (Composition Playlist), inventoried by a **PKL** (Packing List), and located by an **ASSETMAP**. The essence differs because the target is a projector, not a mastering pipeline:
...
**SDH** (Subtitles for the Deaf and Hard of hearing) differs from ordinary translation subtitles: it also carries speaker IDs and non-dialogue sound cues.
